<form class="row g-3 mb-6"Method="POST"action="{{route('user.update',['id'=>$data['user']->id])}}" enctype="multipart/form-data">
    @csrf
    @method('POST')
      ...
      <div class="col-sm-6 col-md-4">
        ...
        <div class="col-sm-6 col-md-4">
          <div class="form-floating">
            <select onchange="change(event)" class="form-select" aria-label="Default select example" name="type">
              <option @if ($data['user']->role=='manager')
                  @selected(true)
              @endif value="manager" >Type User (default manager)</option>
              <option @if ($data['user']->role=='admin')
                @selected(true)
            @endif value="admin">Admin</option>
              <option @if ($data['user']->role=='employee')
                @selected(true)
        ...
        <div @if ($data['user']->role!='employee')
            style="visibility:hidden"
        @endif  class="col-sm-6 col-md-4" id="idma">
          <div class="form-floating">
            <select  class="form-select" id="organizerMultiple" data-choices="data-choices" data-options='{"removeItemButton":true,"placeholder":true}' name="manager">
              <option value="" disabled>Add manager ' your are manger '</option>
              @foreach ($data['mana'] as $item)
    ...
    <div class="col-12 gy-6">
      <div class="row g-3 justify-content-end">
        <div class="col-auto"><button class="btn btn-primary px-5 px-sm-15">Update User</button></div>
      </div>
    </div>
