...
<div class="content">
    <nav class="mb-2" aria-label="breadcrumb">
      <ol class="breadcrumb mb-0">
        ...
        <li class="breadcrumb-item active">Task trash</li>
      </ol>
    </nav>
  ...
  @endif
    <div class="mb-9">
        ...
        <div class="row mb-4 gx-6 gy-3 align-items-center">
            <div class="col-auto">
            <h2 class="mb-0">deleted Task<span class="fw-normal text-700 ms-3">({{$data['count']}})</span></h2>
          </div>
        </div>
          ...
          <div class="col-12 col-sm-auto">
            <div class="d-flex align-items-center">
              <div class="search-box me-3">
                <form method="GET" class="position-relative" action="{{route('task.trash')}}">
                    @csrf
                    <div class="d-flex">
                ...
                @endif
                    ...
                    <div class="avatar-group avatar-group-dense" >
                        ...
                        <div class="avatar avatar-s  rounded-circle">
                            ...
                            src="{{(!(is_null($item->employee->image) || empty($item->employee->image)))?asset("storage/".$item->employee->image):asset("storage/logos/inconnue.jpeg")}}" alt="">
                        </div>
                        </a>
                    </div>
                    @else
                    <p style="margin: auto;text-align: center;"  class="mb-0 fs--1 text-900">none</p>
                    @endif
                    </td>
                    <td class="align-middle white-space-nowrap task ps-3 py-4">
                        <p style="margin: auto;text-align: center;"  class="fw-bo text-900 fs--1 mb-0">
                            {{$item->project ? $item->project->nom : 'Trashed Project' }}
                        </p>
                    </td>
                    <td class="align-middle text-end white-space-nowrap pe-0 action">
                        ...
                        <div class="dropdown-menu dropdown-menu-end py-2">
                            <form  method="POST" action="{{route('task.restore',['id'=>$item->id])}}" >@csrf<input type="hidden" id="id_r" value="{{$item->project ? '0':'1' }}">
                                <button  onclick="btn(event)"  class="dropdown-item" >restore</button></form>
                        </div>
                        </div>
                    </td>
                    </tr>
  ...
  <script>
    function btn(event) {
        ...
        const dd = event.target.form[1].value;
        if (dd == 1) {
            alert('The project is deleted and must be restored');
        } else {
